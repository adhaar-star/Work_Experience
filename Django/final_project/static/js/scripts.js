...
'candidateexpertise[]': { valueNotEquals: "Expertise is required" },
      ...
      alert("chi");
            ...
            data.body['csrfmiddlewaretoken'] = token
      ...
      alert("hi");
                if (data.success) { // if success
                     ...
                     window.location.href =  '/candidate/login';
                      ...
                      alert("lppp");
                 //       $('.loader').hide(); // hide loader
                 toastr.error("Could not Join.PLease retry after some time");
                        $('.submit_btn btn').attr('disabled', false);
                    }
...
$(document).on('click', '.candidate_signup_stepone .question_check a', function() { 
   ...
   $('#chosen_question').val($($(this)["0"].parentElement.parentElement.querySelector("#question_id")).val())
$('.change_ques p')["0"].childNodes["0"].textContent = question

//$("#choosen_question").val($(this)["0"].parentElement.parentElement.childNodes[1].id);
  ...
  experiencenewhtml += '</span><span class="btn_name">Add another</span></a>'
    experiencenewhtml += '</div>'
      ...
      if(imagesize>= 4 * 1024 * 1024){
           ...
           toastr.error("Video Size should be lower than 4MB"); 
           $(file).val("")
        } 
       /* else if(check==false){
           ...
           //    var filetype = obj.data.file_type
                 var imagename =  obj.image_name;  
                  $('.progress-bar').css("width","100%")
                 $('.video_come_here video').attr('src','/media/videos/'+imagename)  
                 videourl+=imagename;
                 $('#uploaded_video').val(imagename);
            ...
            ajax.open("POST",'saveVideo',true); // Your API .net, php
            var uploaderForm = new FormData(); // Create new FormData
            uploaderForm.append("video", elementfiles); // append the next file for upload
            uploaderForm.append("user_id", user_id); // append the next file for upload
            uploaderForm.append("image_name", imagename); // append the next file for upload

            ajax.setRequestHeader("X-CSRFToken",$('input[name ="csrfmiddlewaretoken"]').val());
      ...
      if(imagesize>= 4 * 1024 * 1024){
       ...
       /* else if(check==false){
           ...
           $('.progress-bar').css("width",Math.round(percent)+"%")
            ...
            ajax.open("POST", 'candidate/changeVideo',true); // Your API .net, php
