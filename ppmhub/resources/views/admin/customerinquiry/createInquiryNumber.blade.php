                    ...
                    <div class="col-md-12">
                        <div class="margin-bottom-50 margin-top-25">
                            <table class="tableizer-table table table-bordered text-center" border="1" style="width: 60%;">
                                ...
                                <tbody> 
                                    @foreach($inquirynumber_range as $data)
                                    <tr id="{{$data->id}}">

                                        <td class="startrange">{{$data->start_range}}</td>
                                        <td class="endrange">{{$data->end_range}}</td>
                                        <td>
                                            @if (RoleAuthHelper::hasAccess('inquiryNumber.update')!=true)  
                                            <a href="javascript:void(0)" class="btn btn-default" style="cursor:no-drop; color:#97A7A7;">
                                                @else
                                                <a href="{{url('admin/inquirynumber_range/'.$data->id.'/edit')}}" class="btn btn-info editInquiryrange" id="editInquiryrange_{{$data->id}}" data-id="{{$data->id}}" >
                                                    @endif
                                                    Edit
                                                </a>
                                        </td>                                   
