                 ...
                 <div class="margin-bottom-20">
                    ...
                    <div class="box-header with-border pull-right">
                        <form class="form-inline" method="get">
                            <a style="margin: 0 15px 0 5px; "  href="{{ route('sales-order-inquiry-index')  }}" class="btn btn-warning"><span class="fa fa-refresh"></span></a>
                            ...
                            <div class="form-group">
                                ...
                                |
                            <div class="form-group">
                                <input  value="{{ isset( $_GET['sales_inquiry_no'] )  ? $_GET['sales_inquiry_no']: null }}"  class="datepicker form-control" name="sales_inquiry_no" placeholder="Inquiry No.">
                            </div>
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                @foreach($inquiries as $inquiry)
                                    <tr>
                                        <td>{{$inquiry->sales_inquiry_no}}</td>
                                        <td>{{date("F j, Y ", strtotime($inquiry->created_at))}}</td>
                                        <td style="text-transform: capitalize">{{$inquiry->sales_order_type}}</td>
                                        <td>{{$inquiry->gross_price}}</td>
                                        <td>{{$inquiry->total_price}}</td>
                                        <td  style="text-transform: capitalize">{{$inquiry->status}}</td>
                                        <td>
                                            <a href="{{  route('sales-order-inquiry-items', $inquiry->sales_inquiry_id) }}"><i class="fa fa-eye"></i> View </a>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
