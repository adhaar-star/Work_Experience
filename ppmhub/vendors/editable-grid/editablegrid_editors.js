...
CellEditor.prototype.displayEditor = function (element, editorInput, adjustX, adjustY)
{
    ...
    if (this.editablegrid.editmode == "absolute") {
        ...
        // position input
        var vCenter = this.editablegrid.verticalAlign(element) == "middle" ? (element.offsetHeight - editorInput.offsetHeight) / 2 - paddingTop : 0 + 5;
...
{
    value = (this.type == 'integer' && parseInt(value) < 0) ? parseInt(value) * (-1) : value;
    value = (this.type == 'double' && parseFloat(value) < 0) ? parseFloat(value) * (-1) : value;
    return this.type == 'integer' ? parseInt(value) : parseFloat(value).toFixed(2);
};
...
SelectCellEditor.prototype.isValueSelected = function (htmlInput, optionValue, value) {
...
SelectCellEditor.prototype.getEditor = function (element, value)
{
    ...
    for (var optionIndex = 0; optionIndex < optionValues.length; optionIndex++) {
        ...
        console.log('Option val', optionValue.label, value);
