...
class ProjectStructureController extends Controller
{
    ...
    {
        $chartData = $checklist = $mileStone = array();
        foreach ($taskSubTasks as $tKey => $task) {
            if (array_key_exists('phase_name', $task) && array_key_exists('phase_Id', $task)) {
                ...
                $color = 'dark-purple';
            } else if (array_key_exists('task_name', $task) && array_key_exists('task_Id', $task)) {
                $checklist = projectchecklist::Where('task_id', $task['task_Id'])->get()->toArray();
