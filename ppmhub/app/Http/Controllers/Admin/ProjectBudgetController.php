...
class ProjectBudgetController extends Controller {
  ...
  public function create($module) {
    ...
    $year = $Date->format('Y');
    if ($module == 'original') {
      $project_id = Project:: select(DB::raw("CONCAT(project_Id ,'  (',project_desc,')') AS project_name"), 'id')->where('company_id', Auth::user()->company_id)->pluck('project_name', 'id')->toArray();
      if ($project_id == null)
        $project_id = array();
    } else {
      $project = DB::table('budget_original')
          ->select(DB::raw("CONCAT(project.project_Id ,'  (',project.project_desc,')') AS project_name"), 'budget_original.project_id')
          ->leftJoin('project', 'budget_original.project_id', '=', 'project.id')
          ->where('budget_original.status', '=', 'active')
          ->where('budget_original.company_id', Auth::user()->company_id)
          ->get()->toArray();
  ...
  public function edit($module, $id) {
    ...
    $Date = new \DateTime();
    $year = $Date->format('Y');
    $project_id = Project:: select(DB::raw("CONCAT(id ,'  (',project_name,')') AS project_name"), 'id')->where('company_id', Auth::user()->company_id)->pluck('project_name', 'id')->toArray();
    if ($project_id == null)
      $project_id = array();
    switch ($module) {
      case 'original':
  ...
  public function returnBudget() {
    ...
    $returnBudget = DB::table('budget_return')
      ...
      ->where('budget_return.company_id', Auth::user()->company_id)
  ...
  public function supplementBudget() {
    ...
    $supplementBudget = DB::table('budget_supplement')
      ...
      ->where('budget_supplement.company_id', Auth::user()->company_id)
