...
class PushNotification {
    ...
    public function sendToUserDevices(Array $data) {
        $data['extra'] = (!empty($data['extra'])) ? $data['extra'] : array();
        if (!isset($data['invite'])) {
            $_devices = $this->models->device->getDevices($data['id']);
        } else {
            $_devices = $this->models->device->getStaffDevice($data['room_id']);
        }
        ...
        if (!empty($_devices)) {
            ...
            foreach ($_devices as $_device) {
                ...
                if ($_device['platform'] == 'iOS') {
                    $_notification = array();
                    $notificationDataArray = array('title' => $data['title'], 'soundname' => $_device['ringtone_source'], 'room_id' => !empty($data['staff_id']) ? $data['staff_id'] : $data['room_id'], 'session_type' => !empty($data['session_type']) ? $data['session_type'] : 'room', 'body' => $data['message'], 'badge' => 1, 'icon' => 'cart_only', 'color' => '#ff6c00') + $data['extra'];
                    $notification = array();
                    ...
                    $registationIds = array();
                    $registationIds[] = $_device['notification_id'];
                    $notification_sound = $_device['ringtone_source'];

                    if (!empty($registationIds)) {
                        $notificationDataArray = array('title' => $data['title'], 'soundname' => $notification_sound, 'room_id' => !empty($data['staff_id']) ? $data['staff_id'] : $data['room_id'], 'session_type' => !empty($data['session_type']) ? $data['session_type'] : 'room', 'message' => $data['message'], 'badge' => 1, 'icon' => 'cart_only', 'color' => '#ff6c00') + $data['extra'];
                        $notification = array();
