...
class TimesheetWorkController extends Controller {
    ...
    public function index() {
        ...
        if ($user_role_id != 2) {
            ...
            $employees_data = Employee_records::ByCompany()->orderBy('employee_first_name', 'asc')->get();


            $first_employee = !empty($employees_data->count()) ? $employees_data : null;
            
            $employee_id = $first_employee->employee_id;
            
            $login_employee_data = Employee_records::where('company_id', Auth::user()->company_id)->where('employee_id',$employee_id)->first();
    ...
    public function timesheetentry_save() {
        ...
        if (!empty($project_ids)) {
            ...
            if ($week_save_status) {
                 ...
                 */
                if ($check) {
                    $this->request->session()->flash('alert-success', 'Time Sheet entry was successfully added!');
                } else {
                    $this->request->session()->flash('alert-danger', 'Time Sheet entry was not successfully added!');
                }
                // code ends here
            } else {
                $this->request->session()->flash('alert-danger', 'Time Sheet entry was not successfully added!');
