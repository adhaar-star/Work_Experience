...
@section('title','Settings | Person Responsible')
@section('body')
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <div class="dashboard-buttons">
                    @if (RoleAuthHelper::hasAccess('personResponsible.create')!=true)  
                    <a href="javascript:void(0)" class="btn btn-default" style="cursor:no-drop; color:#97A7A7;">
                        @else
                        <a href="{{url('admin/personresponsible/create')}}" class="btn btn-primary">
                            @endif
                            <i class="fa fa-send margin-right-5"></i>
                            Create Person Responsible
                        </a>
                </div>
                ...
                <div class="col-md-12">
                    <div class="margin-bottom-50">
                        <table class="table table-inverse" id="example3" width="100%">
                            ...
                            <tbody>
                                @foreach($personresponsible as $ptype)
                                <tr>
                                    <td>{{$ptype->name }}</td>
                                    ...
                                    <td class="action-btn">
                                        @if (RoleAuthHelper::hasAccess('personResponsible.update')!=true)  
                                        <a href="javascript:void(0)" class="btn btn-default btn-xs margin-right-1" style="cursor:no-drop; color:#97A7A7;">
                                            @else
                                        <a href="{{url('admin/personresponsible/'.$ptype->id.'/edit')}}" class="btn btn-info btn-xs margin-right-1">@endif<i class="fa fa-pencil"></i>  </a>

                                        {!! Form::open(array('route' => array('personResponsible.delete',$ptype->id), 'method' => 'DELETE','id'=>'delform'.$ptype->id)) !!}
                                        @if (RoleAuthHelper::hasAccess('personResponsible.delete')!=true)  
                                        <a href="javascript:void(0)" class="btn btn-default btn-xs margin-right-1" style="cursor:no-drop; color:#97A7A7;">
                                            @else
                                        <a href="javascript:void(0)" onclick="var res = confirm('Are you sure you want to delete this person responsible');
                                                  if (res) {
