<table class="table table-striped">
    ...
    <tbody>
        @forelse($orderItems as $item)
            <tr>
                <td>{{ $item->sales_order_item_no }}</td>
                ...
                <td>
                    <button class="btn btn-info btn-xs margin-right-1" data-toggle="modal" data-target="#table-view-popup_{{ $item->sales_order_item_id }}"><i class="fa fa-eye" aria-hidden="true"></i></button>
                    @if($order->sales_order_status == 'created')
                        <a href="{{ route('sales-order-items', $item->sales_order_id) }}?update_item={{ $item->sales_order_item_id }}" class="btn btn-primary btn-xs margin-right-1"><i class="fa fa-pencil"></i> <!--Edit--> </a>
                        <button  class="btn btn-danger btn-xs margin-right-1 deleteGlobalBtn" data-url="{{ route('sales-order-items-delete', $item->sales_order_item_id ) }}"  ><i class="fa fa-trash-o"></i></button>
                    @endif
                    @if($order->sales_order_status == 'approved')
                        @if($item->sales_order_status == 'created')
                            <button  class="btn btn-success btn-xs margin-right-1 deliveryGlobalBtn" data-url="{{ route('sales-order-items-delivery', $item->sales_order_item_id ) }}"  ><i class="fa fa-truck"></i></button>
                        @endif
                        @if($item->sales_order_status == 'delivery')
                            <button  class="btn btn-success btn-xs margin-right-1" title="{{   $item->delivery_info }}"><i class="fa fa-check"></i></button>
                        @endif
                    @endif
                    <div class="modal fade table-view-popup" id="table-view-popup_{{ $item->sales_order_item_id }}" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
                        <div class="modal-dialog" role="document" style="text-align:left;">
                            <div class="modal-content" style="border-radius: 0;">
                                <div class="modal-header">
                                    <h3 class="model-list-striped-title">Sales Order Item</h3>
                                ...
                                <div class="modal-body model-list-striped">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-8">
                                                <p class="form-control-static">{{ $item->sales_order_item_no }}</p>
                                            </div>
                                        </div>
                                        @if($item->sales_order_status == 'delivery')
                                        <div class="form-group">
                                            <div class="col-sm-4">
                                                <p class="form-control-static">Delivery Information</p>
                                            </div>
                                            <div class="col-sm-8">
                                                <p class="form-control-static">{{ $item->delivery_info }}</p>
                                            </div>
                                        </div>
                                        @endif
                                        <div class="form-group">
                                ...
                                <div class="modal-footer">
                                    @if($order->sales_order_status == 'created' )
                                        <span class="edit-btn"><a href="{{ route('sales-order-items', $item->sales_order_id) }}?update_item={{ $item->sales_order_item_id }}" class="btn btn-primary">Edit Item</a></span>
