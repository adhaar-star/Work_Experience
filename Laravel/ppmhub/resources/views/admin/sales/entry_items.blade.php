            ...
            <div class="col-lg-8">
                @if(!empty($order))
                    @if($order->sales_order_status == 'created')
                            ...
                            <div class="card salesItemFormContent" @if(empty($updateItem) &&  !empty($orderItems->count())) style="display: none; margin-bottom: 60px;" @endif>
                                <div class="card-block">
                                    <div class="tab-content">
                                        @include('admin.sales.include.order_entry_item')
                                    </div>
                                </div>
                            </div>
                        @endif
                        @if($order->sales_order_type == 'service')
                            <div class="card salesItemFormContent" @if(empty($updateItem) &&  !empty($orderItems->count())) style="display: none; margin-bottom: 60px;" @endif>
                                <div class="card-block">
                                    <div class="tab-content">
                                        @include('admin.sales.include.order_entry_item_service')
                                    </div>
                                </div>
                            </div>
                        @endif
                        @if( $order->sales_order_type == 'milestone'  || $order->sales_order_type == 'timesheet' )
                            <div class="card salesItemFormContent" @if(empty($updateItem) &&  !empty($orderItems->count())) style="display: none; margin-bottom: 60px;" @endif>
                                <div class="card-block">
                                    <div class="tab-content">
                                        @include('admin.sales.include.order_entry_item_project')
                                    </div>
                                </div>
                            </div>
                        @endif
                    ...
                    @if(!empty($orderItems->count()))
                            <div class="card">
                                <div class="card-header"><span class="highlightCustom"> {{ $order->sales_order_type  }} Billing</span>
                                    @if($order->sales_order_status == 'created')
                                        @if((empty($updateItem)))
                                        ...
                                        @endif
                                    @else
                                        <button  style="margin: -5px 0 0;" class="btn btn-sm btn-success pull-right">Sales Order No. {{ $order->sales_order_no  }}</button>
                                    @endif
                                </div>
                                <div class="card-block">
                                    <div class="tab-content">
                                        @include('admin.sales.include.order_items')
                                    </div>
                                </div>
                            </div>
                    @endif
                            ...
                            <div class="row">
                                 <table class="table table-striped">
                                    ...
                                    <tr class="bg-info">
                                     ...
                                     @endif
                                         <tr>
                                             @if($order->sales_order_status != 'billed' && $order->total_price > 0 )
                                             <td colspan="2">Current Order Status: <div class="label label-warning" style="margin-right: 15px; text-transform: capitalize;">{{ $order->sales_order_status }}</div>
                                                @if($order->sales_order_status == 'created' && $order->total_price > 0 )
                                                    <button type="button"  data-toggle="modal" data-target="#SubmitForApproved" class="btn btn-primary pull-right">Submit for Approval</button>
                                               @endif
                                             </td>
                                             @endif
                                             @if($order->sales_order_status == 'billed' && $order->total_price > 0 )
                                                 <td>Current Order Status: <div class="label label-success" style="margin-right: 15px; text-transform: capitalize;">{{ $order->sales_order_status }}</div></td>
                                                 <td><a href="{{ route('billing-single-view', $order->billing->billing_id  ) }}"> Billing No. {{ $order->billing->billing_no }} </a></td>
                                             @endif
                                ...
                                @if($order->sales_order_status == 'created' )
                                <a href="{{ route('sales-order-edit', $order->sales_order_id ) }}"  style="margin-left: 15px;" class="btn btn-sm btn-primary">Update Order Information</a>
                                @else
                                ...
                                @endif
                                <button  style="margin-right: 15px;" class="btn btn-sm btn-success pull-right">Sales Order No. {{ $order->sales_order_no  }}</button>
...
<div class="modal fade table-view-popup" id="SubmitForApproved" tabindex="-1" role="dialog" aria-labelledby="" aria-hidden="true">
    <div class="modal-dialog" role="document" style="text-align:left;">
        <div class="modal-content" style="border-radius: 0;">
            ...
            <div class="modal-footer">
                {!! Form::open(['url' => route('sales-order-submit', $order->sales_order_id ),  'class' => 'form-horizontal GlobalFormValidation']) !!}
                <div class="row">
                    <div class="col-sm-10 col-lg-offset-1">
                        @include('layout.admin_layout_include.alert_process')
                    </div>
            ...
            <div class="modal-body">
                <div class="row">
                    ...
                    <div class="col-md-6" style="text-align: center;">
                        ...
                        {!! Form::open(['url' => route('sales-order-reject',  $order->sales_order_id),  'class' => 'form-horizontal GlobalFormValidation ']) !!}
                    ...
                    <div class="col-md-6"  style="text-align: center;">
                        ...
                        {!! Form::open(['url' => route('sales-order-approve',  $order->sales_order_id),  'class' => 'form-horizontal GlobalFormValidation ']) !!}
        ...
        <div class="modal-content" style="border-radius: 0;">
            ...
            <div class="modal-body model-list-striped">
                ...
                <div class="form-group">
                    <div class="col-sm-4">
                        <p class="form-control-static">Sales Order No.</p>
                    </div>
                    <div class="col-sm-8">
                        <p class="form-control-static">{{ $order->sales_order_no }}</p>
                    </div>
                </div>
