...
@section('title','Approvers Edit Form')
@section('body')
@if (count($errors) > 0)
	...
	<section class="panel">
            <div class="panel-body">
                <div class="row">
                    ...
                    <form id="approver_form" method="post" action="{{url('admin/timesheetapprover_edit_save/'.$timesheet_approver->id)}}" data-parsley-validate="" class="form-horizontal form-label-left" novalidate="">
                      {{ csrf_field() }}
										...
										<div class="col-xs-12 col-sm-9">
											<select class="form-control" id="time_sheet_user_id" name="time_sheet_user_id">
											  ...
											  @foreach($employees_list as $employee)
											  {
												<?php
													$time_sheet_user_id='';
													if($employee->employee_id==$timesheet_approver->time_sheet_user_id)
													{
														$time_sheet_user_id='selected';
													}
												?>
												
												<option value="{{$employee->employee_id}}" <?php echo $time_sheet_user_id; ?>>{{$employee->employee_first_name}} {{$employee->employee_middle_name}} {{$employee->employee_last_name}}</option>
											  }	
											  @endforeach
											</select>
										</div>
											...
											<select class="form-control" id="time_sheet_approver_id" name="time_sheet_approver_id">
											  ...
											  @foreach($employees_list as $employee)
											  {
												...
												<?php
													...
													if($employee->employee_id==$timesheet_approver->time_sheet_approver_id)
													{
														$time_sheet_approver_id='selected';
													}
												?>
											  
												<option value="{{$employee->employee_id}}" <?php echo $time_sheet_approver_id; ?>>{{$employee->employee_first_name}} {{$employee->employee_middle_name}} {{$employee->employee_last_name}}</option>
											  }	
											  @endforeach
											</select>
										</div>
