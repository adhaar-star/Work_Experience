    ...
    $('#discount').change(function (evt) {
        setTimeout(function () {
            if ($("#discount").val() == '' || $("#discount").val() == undefined || $("#discount").val() == 'undefined') {
                ...
                $("input[name = 'salesorder_discount']").val('');
                $("input[name = 'salesorder_discount_amt']").val('');
                $("input[name = 'salesorder_discount_gross_price']").val('');
                $("input[name = 'salesorder_sales_taxamt']").val('');
                $("input[name = 'salesorder_net_price']").val('');
                $("input[name = 'salesorder_freight_charges']").val('');
                $("input[name = 'salesorder_total_price']").val('');
                calculate(evt);
            }
    ...
    $('#purchase_item_form .form').each(function (i, row) {
        if (!isNaN(row.querySelector('[name=gross_price]').value))
        {
            ...
            if (!isNaN(row.querySelector('[name=discount]').value))
            {
                ...
                if (!isNaN(discount))
                    $('[name=salesorder_discount]').val(discount.toFixed(2));
            }
            if (!isNaN(row.querySelector('[name=profit_margin]').value))
            {
                ...
                if (!isNaN(profit))
                    $('[name=salesorder_profit_margin]').val(profit.toFixed(2));
            }
            if (!isNaN(quotation_gross_price))
                $('[name=salesorder_gross_price]').val(Math.round(quotation_gross_price));
        }
        if (!isNaN(row.querySelector('[name=profit_amt]').value))
        {
            profit_amt += parseInt(row.querySelector('[name=profit_amt]').value);
            if (!isNaN(profit_amt))
                $('[name=salesorder_profit_amt]').val(Math.round(profit_amt));
        }
        if (!isNaN(row.querySelector('[name=profit_gross_price]').value))
        {
            ...
            if (!isNaN(profit_gross_price))
                $('[name=salesorder_profit_margin_grossprice]').val(Math.round(profit_gross_price));
        }
        if (!isNaN(row.querySelector('[name=discount_amt]').value))
        {
            ...
            if (!isNaN(discount_amt))
                $('[name=salesorder_discount_amt]').val(Math.round(discount_amt));
        }
        ...
        if (!isNaN(row.querySelector('[name=freight_charges]').value))
        {
            ...
            if (!isNaN(freight_charges))
                $('[name=salesorder_freight_charges]').val(Math.round(freight_charges));
        }
        if (!isNaN(row.querySelector('[name=total_price]').value))
        {
            total_price += parseInt(row.querySelector('[name=total_price]').value);
            if (!isNaN(total_price))
                $('[name=salesorder_total_price]').val(Math.round(total_price));
        }
    });
