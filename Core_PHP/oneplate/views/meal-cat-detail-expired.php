	...
	function change()
{
  ...
  return query_string;
	...
	var request = $.ajax({
  url: "meal-cat-detail.php",
  type: "GET",
 data: {id : activeid,search : queryparam},
  dataType: "html"
});
...
<div class="row">
	<button class='btn btn-primary pushme' id="myButton1" style="float: right;" onclick="change()" >Active List</button>
...
if($search==""){
                $active_listings = $db->get_category_expired_listings($pagination->get_limit(), $pagination->get_start(),$categoryid);
}
else{
  $active_listings = $db->get_category_search_expired_listings($pagination->get_limit(), $pagination->get_start(),$categoryid,$search);
