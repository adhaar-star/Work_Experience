	...
	function change()
{
  ...
  return query_string;
	...
	var request = $.ajax({
  url: "meal-cat-detail-expired.php",
  type: "GET",
  data: {id : expiredid,search : queryparam},
  dataType: "html"
});
...
if($search==""){
                $active_listings = $db->get_category_active_listings($pagination->get_limit(), $pagination->get_start(),$categoryid);
}
else{
  $active_listings = $db->get_category_search_active_listings($pagination->get_limit(), $pagination->get_start(),$categoryid,$search);
//echo count($active_listings);die;
}
		...
		<?php
                        ...
                        '</td>';
                        echo '<td>' .
                        anchor(['href' => 'meal-cat-detail.php?id=' . $categoryid,
                        ...
                        echo '<td><button type="button" class="btn btn-xs btn-primary center-block" data-name="',$record['user_name'],'"
                        data-location="',$record['location'],'"
                        data-category="',$record['meal_name'],'"
                        data-time="',$db->get_remaining_time($record['expire_after']),'"
                        data-description="',$record['description'],'"
                        data-phone="',$record['telephone'],'" data-toggle="modal" data-target="#quoteView"><span class="glyphicon glyphicon-fullscreen"></span></button></td>';
                        echo '</tr>';
                    }
                } else {
                    echo '<tr>'
