...
<?php if ( isset( $mcq['settings']['others'] ) && $mcq['settings']['others'] == true && $do_data ) : ?>
...
<?php if ( isset( $mcq['settings']['show_feedback'] ) && $mcq['settings']['show_feedback'] == true && $do_data ) : ?>
<?php $this->ui->collapsible_head( $mcq['settings']['feedback_label'] ); ?>
<table class="ipt_fsqm_preview others">
	<thead>
		<tr>
	...
	<tbody>
		<tr class="empty">
			<td colspan="4"><?php _e( 'No data yet!', 'ipt_fsqm' ); ?></td>
		</tr>
	</tbody>
</table>
<?php $this->ui->collapsible_tail(); ?>
...
<table class="ipt_fsqm_preview table_to_update">
	...
	<tbody>
		...
		<tr>
			<td style="width: 50%" class="visualization row-<?php echo $r_key; ?>" rowspan="<?php echo ( count( (array) $mcq['settings']['columns'] ) * count( (array) $options_array ) ); ?>"><!-- Combo --></td>
		<?php $last_foreach = 0; foreach ( (array) $mcq['settings']['columns'] as $c_key => $column ) : $last_foreach++; ?>
			<th style="width: 20%" rowspan="<?php echo count( $options_array ); ?>"><?php echo $column; ?></th>
		<?php $last_op_foreach = 0; foreach ( $options_array as $o_key => $op ) : $last_op_foreach++; ?>
