...
class IPT_FSQM_Form_Category extends IPT_FSQM_Admin_Base {
	 ...
	 *========================================================================*/
	public function admin_menu() {
		$this->pagehook = add_submenu_page( 'ipt_fsqm_dashboard', __( 'Form Categories', 'ipt_fsqm' ), __( 'Form Category' ), $this->capability, 'ipt_fsqm_form_category', array( $this, 'index' ) );
		parent::admin_menu();
	}

	public function index() {
		global $ipt_fsqm_info;
		$this->index_head( __( 'WP Feedback, Survey & Quiz Manager Pro <span class="ipt-icomoon-arrow-right2"></span> Form Category <a href="admin.php?page=ipt_fsqm_form_category&paction=new_cat" class="add-new-h2">Add New</a>', 'ipt_fsqm' ), false );
...
<div class="ipt_uif_iconbox ipt_uif_shadow glowy">
	...
	protected function category_form( $id = '' ) {
		$buttons = array(
			array( __( 'Create Category', 'ipt_fsqm' ), '', 'large', 'primary', 'normal', array(), 'submit' ),
			array( __( 'Reset', 'ipt_fsqm' ), '', 'small', 'secondary', 'normal', array(), 'reset' ),
			array( __( 'View All', 'ipt_fsqm' ), '', 'small', 'secondary', 'normal', array(), 'anchor', array(), array(), admin_url( 'admin.php?page=ipt_fsqm_form_category' ) ),
		);
		if ( $id == '' ) {
...
class IPT_FSQM_Data_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		//prepare our query
		$query = "SELECT d.id id, d.f_name f_name, d.l_name l_name, d.email email, d.phone phone, d.ip ip, d.date date, d.star star, d.score score, d.max_score max_score, d.user_id user_id, d.url_track track, f.name name, f.id form_id, c.name catname FROM {$ipt_fsqm_info['data_table']} d LEFT JOIN {$ipt_fsqm_info['form_table']} f ON d.form_id = f.id LEFT JOIN {$ipt_fsqm_info['category_table']} c ON f.category = c.id";
...
<div class="alignleft actions">
	...
	<select name="track_id" id="track_id">
		<option value=""<?php if ( !isset( $_GET['cat_id'] ) || '' == $_GET['cat_id'] ) echo ' selected="selected"'; ?>><?php _e( 'Show all URL Tracks', 'ipt_fsqm' ); ?></option>
...
class IPT_FSQM_Category_Table extends WP_List_Table {
	...
	public function prepare_items() {
		...
		// Prepare our query
		$query = "SELECT id, name, description FROM {$ipt_fsqm_info['category_table']}";
