...
<div class="ipt_fsqm_user_portal ipt_uif_front ipt_uif_common" data-ui-theme="<?php echo esc_attr( json_encode( $theme_element['include'] ) ); ?>" data-ui-theme-id="<?php echo esc_attr( $theme ); ?>" data-settings="<?php echo esc_attr( json_encode( $shortcode_settings ) ); ?>" data-nonce="<?php echo esc_attr( wp_create_nonce( 'ipt_fsqm_up_nonce_' . $user->ID ) ); ?>" data-ajaxurl="<?php echo esc_attr( admin_url( 'admin-ajax.php' ) ); ?>">
	...
	public static function user_portal_ajax_response() {
		...
		foreach ( $data_ids as $id ) {
			...
			$return['html'] .= '<td class="date_label" data-order="' . strtotime( $data->data->date ) . '">' . date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $data->data->date ) ) . '</td>';
			if ( $settings['showcategory'] == '1' ) {
				$category = IPT_FSQM_Form_Elements_Static::get_category( $data->category );
				if ( $category != null ) {
					$return['html'] .= '<td class="category_label">' . $category->name . '</td>';
				} else {
					$return['html'] .= '<td class="category_label">' . __( 'None', 'ipt_fsqm' ) . '</td>';
				}
...
<?php if ( $form->settings['trackback']['show_trends'] == true ) : ?>
<?php echo do_shortcode( '[ipt_fsqm_trends form_id="' . $form->form_id . '"]' ); ?>
<?php endif; ?>
		<?php
	}
			...
			if ( is_array( $survey ) && !empty( $survey ) ) {
				foreach ( $survey as $m_key => $element ) {
					...
					case 'thumbselect' :
						...
						if ( isset( $data->data->mcq[$m_key]['others'] ) && ! empty( $data->data->mcq[$m_key]['others'] ) && $do_data ) {
							$return['survey']["$m_key"]['others_data'][] = array(
					...
					case 'smileyrating' :
						...
						if ( $do_data && isset( $data->data->mcq[$m_key]['feedback'] ) && $data->data->mcq[$m_key]['feedback'] != '' ) {
							$return['survey']["$m_key"]['feedback_data'][] = array(
				...
				foreach ( $feedback as $f_key => $element ) {
					...
					case 'gps' :
						...
						$return['feedback']["$f_key"][] = array(
							...
							'date'          => date_i18n( get_option( 'date_format' ) . __(' \a\t ', 'ipt_fsqm') . get_option( 'time_format' ), strtotime( $data->data->date ) ),
							'id'            => $data->data_id,
							'map'           => sprintf( '//maps.googleapis.com/maps/api/staticmap?markers=%1$s,%2$s&zoom=%3$s&size=500x300&scale=2', round( (float) $data->data->freetype[$f_key]['lat'], 6 ), round( (float) $data->data->freetype[$f_key]['long'], 6 ), $data->freetype[$f_key]['settings']['zoom'] ),
	...
	public static function delete_categories( $ids ) {
		...
		do_action( 'ipt_fsqm_form_category_deleted', $ids, $return );
		return $return;
	}
