...
class IPT_FSQM_Loader {
	...
	public function __construct( $file_loc, $text_domain = 'default', $version = '1.0.0', $abbr = '', $doc = '', $sup = '' ) {
		...
		self::$abbr = $abbr;
		self::$init_classes = array( 'IPT_FSQM_Dashboard', 'IPT_FSQM_All_Forms', 'IPT_FSQM_New_Form', 'IPT_FSQM_Import_Export', 'IPT_FSQM_Form_Category', 'IPT_FSQM_Report', 'IPT_FSQM_View_Submission', 'IPT_FSQM_View_All_Submissions', 'IPT_FSQM_Settings' );
	...
	public function popup_forms() {
		...
		wp_enqueue_script( 'jquery-remodal', plugins_url( '/lib/js/jquery.remodal.min.js', IPT_FSQM_Loader::$abs_file ), array( 'jquery' ), IPT_FSQM_Loader::$version );
		wp_enqueue_script( 'ipt-fsqm-modal-popup', plugins_url( '/static/front/js/ipt-fsqm-modal-popup.js', IPT_FSQM_Loader::$abs_file ), array( 'jquery', 'jquery-remodal' ), IPT_FSQM_Loader::$version );
		wp_enqueue_style( 'jquery-remodal-css', plugins_url( '/lib/css/jquery.remodal.css', IPT_FSQM_Loader::$abs_file ), array(), IPT_FSQM_Loader::$version );
		...
		foreach ( $ipt_fsqm_popup_forms as $form_id ) {
			$form = new IPT_FSQM_Form_Elements_Front( null, $form_id );
			?>
<div class="remodal" data-remodal-id="ipt-fsqm-popup-form-<?php echo $form_id ?>">
	<div class="ipt-fsqm-pf-inner">
		<?php $form->show_form(); ?>
	</div>
</div>
			<?php
