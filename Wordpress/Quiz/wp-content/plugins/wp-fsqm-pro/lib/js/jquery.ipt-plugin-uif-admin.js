...
(function($) {
	...
	var methods = {
		...
		applySlider : function() {
			...
			if(slider_range) {
				first_input.change(function() {
					...
					count_div.find('span.ipt_uif_slider_count_min').text(parseFloat(first_input.val()));
				});
				$(second_input).change(function() {
					slider.slider({
						values : [parseFloat(first_input.val()), parseFloat(second_input.val())]
					});
					count_div.find('span.ipt_uif_slider_count_max').text(parseFloat(second_input.val()));
				});
			} else {
				first_input.change(function() {
					slider.slider({
					...
					count_div.find('span').text(parseFloat(first_input.val()));
				});
			}
		},
		...
		builderSettingsClose : function(settings_box, container) {
			...
			//Check wp_editor
			var wp_editor_textarea = $(settings_box).find('textarea.wp_editor').eq(0);
			if( wp_editor_textarea.length && typeof(tinyMCE) == 'object' ) {
