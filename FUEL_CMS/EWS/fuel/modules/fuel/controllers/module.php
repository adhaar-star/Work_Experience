	...
	function edit($id = NULL, $field = NULL, $redirect = TRUE)
	{
		...
		if ($this->input->post($this->model->key_field()))
		{
			if ($this->_process_edit($id) AND !has_errors())
			{
				if ($inline === TRUE)
				{
					$url = fuel_uri($this->module_uri.'/'.$id.'/'.$field);
				}
				else
				{
...
if($url='http://localhost/ews/index.php/fuel/contractors/edit/$id')
{
$this->fuel->admin->render('module_edit',$vars);
}
else
{
$this->fuel->admin->render('module_edit',$vars);
...
if($this->preview_path="fuel/jobs/view/{id}")
{
	// check that the action even exists and if not, show a 404
	...
	$CI =& get_instance();
 $this->db->select('id,job_id,Old_status,New_status');
	 
 $this->db->from('fuel_status_history');
...
$this->fuel->admin->render('view',$vars);
