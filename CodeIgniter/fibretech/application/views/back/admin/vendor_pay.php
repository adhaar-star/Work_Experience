            ...
            <div class="table-responsive">
                <table class="table table-striped">
                    <tr>
                        ...
                        <td><?php $total_sold = $this->crud_model->vendor_share_total($vendor_id); echo currency().$total_sold['total']; ?></td>
                    </tr>
                    <tr>
                        <td><?php echo translate('paid_by_customer');?></td>
                        <td><?php $total_payable = $this->crud_model->vendor_share_total($vendor_id,'paid'); echo currency().$total_payable['total']; ?></td>
                    </tr>
                    <tr>
                        <td><?php echo translate('paid_to_vendor');?> (<?php echo translate('by_admin');?>)</td>
                        <td><?php echo currency().$already_paid = $this->crud_model->paid_to_vendor($vendor_id); ?></td>
                    </tr>
                    <tr>
                        <td><?php echo translate('paid_to_vendor');?> (<?php echo translate('cash_on_delivery');?>)</td>
                        <td><?php $cod_paid = $this->crud_model->vendor_share_total($vendor_id,'paid','cash_on_delivery'); echo currency().$cod_paid['total']; ?></td>
                    </tr>
                    <tr>
                        <td><?php echo translate('due_to_vendor');?></td>
                        <td><?php echo currency().$amount = ($total_payable['total'] - $already_paid - $cod_paid['total']); ?></td>
                    </tr>
                </table>
            </div>

                <?php //echo currency().$this->crud_model->total_sale($row['vendor_id']); ?>
            
            <div class="form-group">
                <label class="col-sm-4 control-label" for="demo-hor-1"><?php echo translate('amount');?> (<?php echo currency(); ?>)</label>
                <div class="col-sm-6">
