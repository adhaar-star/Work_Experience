...
?>
<script>
    var del_url = '<?php echo base_url() ?>index.php/users/deleteAll';
    var tag_url = '<?php echo base_url() . 'index.php/users/index/' . $this->uri->segment(3) . $suffixUrl; ?>';
    var sort_url = '<?php echo base_url() ?>index.php/users/sortData/<?php echo $this->uri->segment(3) . $suffixUrl; ?>';
</script>
...
if ($users) {
    $cnt = count($users);
    $startId = $users[0]->user_id;
    $endId = $users[$cnt - 1]->user_id;
}
?>
...
<!--                <input class="btn btn-primary" onclick="window.location = '<?php echo base_url() ?>index.php/users_bank_details/add'"  type="button" value="Add New Skill" style="margin-bottom:5px">
            ...
            <div class="col-md-5 col-md-5 pull-right tabmt10">
                <div class="row">
                    <form action="<?php echo base_url() . 'index.php/users' ?>" method="get">
            ...
            <div class='col-sm-12'>
                <div class='box bordered-box blue-background' style='margin-bottom:0;'>
                    <div class='box-header blue-background pull-left col-xs-12 ofv' id="boxHeader">
                        <div class='title'>All Users Details</div>
                    </div>
...
<!--                                            <th>
                                            ...
                                            <th data-page="all" data-sort="DESC" dta-name="email" class="active sort_column" data-startId="<?php echo $startId; ?>" data-endId="<?php echo $endId; ?>">
...
//                                                echo "<td><input type='checkbox' class='del_id' name='del_id[]' value='" . $row->skill_id . "'></td>";
                                                ...
                                                if ($row->profile_pic != '') {
                                                    ...
                                                    if ($filename > 0) {
                                                        echo "<td align='center'><img src='" . USER_IMAGES . '/' . $row->profile_pic . "' height='50px' width='50px'></td>";
                                                    } else {
                                                ...
                                                echo "<td><center>";
                                                echo "<a href='" . base_url() . "index.php/users/verifiedByAdmin/" . $row->user_id . "' class='del btn btn-warning btn-xs'  onclick='return changeStatusPopup(this)'";
                                               if ($row->is_verified == 0) {
                                                    echo "title='Verify User'";
                                                    echo "><i class='icon-ok'></i>";
                                                } else {
                                                    echo "title='Unverify User'";
...
////                                                echo "<a href='" . base_url() . "index.php/skills/edit/" . $row->skill_id . "' class = 'btn btn-success  btn-xs' title='Edit Skill'><i class='icon-edit'></i></a>  ";
                                                echo "<a href='" . base_url() . "index.php/users/view/" . $row->user_id . "' class = 'btn btn-info  btn-xs' title='View User'><i class='icon-eye-open'></i></a>  ";
...
<script>
    ...
    function deletePopup(obj) {
        jconfirm("Do you really want to delete this User?", function (r) {
            console.log(r);
            if (r) {
                window.location = $(obj).attr('href');
            }
