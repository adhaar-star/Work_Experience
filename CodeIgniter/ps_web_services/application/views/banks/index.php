...
?>
<script>
    var del_url = '<?php echo base_url() ?>index.php/banks/deleteAll';
    var tag_url = '<?php echo base_url() . 'index.php/banks/index/' . $this->uri->segment(3) . $suffixUrl; ?>';
    var sort_url = '<?php echo base_url() ?>index.php/banks/sortData/<?php echo $this->uri->segment(3) . $suffixUrl; ?>';
</script>
...
if ($banks) {
    $cnt = count($banks);
    $startId = $banks[0]->bank_id;
    $endId = $banks[$cnt - 1]->bank_id;
}
?>
    ...
    <div class='col-xs-12'>
        ...
        <div class='row'>
            ...
            <div class="col-md-5 col-md-5 pull-right tabmt10">
                <div class="row">
                    <form action="<?php echo base_url() . 'index.php/banks' ?>" method="get">
                                ...
                                <table class='data-table table table-bordered' style='margin-bottom:0;'>
                                    <thead>
                                        <tr>
                                            ...
                                            <th data-page="all" data-sort="DESC" dta-name="bank_name" class="active sort_column" data-startId="<?php echo $startId; ?>" data-endId="<?php echo $endId; ?>">
...
//                                            $s = 1;
                                            foreach ($banks as $row) {
                                                echo "<tr>";
                                                echo "<td><input type='checkbox' class='del_id' name='del_id[]' value='" . $row->bank_id . "'></td>";
                                                echo "<td>" . $sr . "</td>";
                                                ...
                                                echo "<td><center>";
                                                echo "<a href='" . base_url() . "index.php/banks/changeStatusByAdmin/" . $row->bank_id . "' class='del btn btn-warning btn-xs'  onclick='return changeStatusPopup(this)'";
                                                if ($row->bank_active == 1) {
                                                    echo "title='Activate Bank'";
                                                    echo "><i class='icon-ok'></i>";
                                                } else {
                                                    echo "title='Deactivate Bank'";
                                                    echo "><i class='icon-remove'></i>";
                                                }
                                                echo "</a>  ";
                                                echo "<a href='" . base_url() . "index.php/banks/edit/" . $row->bank_id . "' title='Edit Bank' class='btn btn-success btn-xs'><i class='icon-edit'></i></a>  ";
                                                echo "<a id='alert-example' href='" . base_url() . "index.php/banks/delete/" . $row->bank_id . "' title='Delete Bank' class='del btn btn-danger btn-xs' onclick='return deletePopup(this)'><i class='icon-trash'></i></a>";
                                                echo "</center></td>";
                                                echo "</tr>";
                                                $sr++;
                                            }
                                        } else {
                                            echo "<tr><td colspan='10' align='center'>No Bank found</td></tr>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
...
<script>
    ...
    function deletePopup(obj) {
        jconfirm("Do you really want to delete this Bank?", function (r) {
            console.log(r);
            if (r) {
                window.location = $(obj).attr('href');
            }
