...
class Payment extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->input->get('type') && $this->input->get('keyword')) {
            ...
            $type = $this->input->get('type');
            if ($type == 1) {
                $con = 'Where payment_requested=1 and release_requested=0 and payment_release=0 ORDER BY p.payment_date DESC';
				
            } else if ($type == 2) {
                $con = ' Where release_requested=1 and payment_release=0 ORDER BY p.payment_date DESC';
			
            } else if ($type == 3) {
                $con = ' Where payment_release=1 ORDER BY p.payment_release_date DESC';
				
            }
        } else {
            $con = 'Where payment_requested=1 or release_requested=1 ORDER BY p.payment_date DESC';
			
        }
//$con='ORDER BY p.payment_release_date DESC';
        $query = "select * from " . $this->table . " p Left join " . TBL_USERS . " u on u.user_id = p.freelancer_id " . $con;
