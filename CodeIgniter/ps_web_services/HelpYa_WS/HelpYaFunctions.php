...
class HelpYaFunctions
{
        ...
        switch ($service)
        {
					...
					case "AddUserImage":
            {
            	return $this->AddUserImage($postData);
					...
					case "UpGradeUser":
            {
            	return $this->UpGradeUser($postData);
            }	
						case "UserUpdateStatus":
            {
            	return $this->UserUpdateStatus($postData);
            }	
        }
    }
    ...
    public function RegisterUser($UserData)
    {
        ...
        $querySelNor = "Select * from tbluserdetails where email = '$email' OR  user_name='$name'";
        ...
        {
            $status = 2;
            $errorMsg = "Email Id or Username already exists";
...
public function  UpGradeUser($postData)
    {
...
$expirytime=$date->format("Y-m-d h:i:s");
        ...
        $insertDeviceQuery = "UPDATE  tbluserdetails SET `upgrade`=1,`user_expiry`='$expirytime' WHERE `user_id`='$user_id'";
        $insertDeviceResult = mysql_query($insertDeviceQuery) or $errorMsg =  mysql_error();
	
	if ($insertDeviceResult)
        {
	 $status = 1;
      $errorMsg = "Successfully upgraded the user";
		
	}
	else{
	
	$status = 0;
            $errorMsg = 'Error upgrading user';
	}
	...
	public function  UserUpdateStatus($postData)
    {
	 $user_id =  $postData->user_id;

	
	
        $insertDeviceQuery = "Select * FROM tbluserdetails WHERE `user_id`='$user_id'";
        $insertDeviceResult = mysql_query($insertDeviceQuery) or $errorMsg =  mysql_error();
	
	
                while($skill = mysql_fetch_assoc( $insertDeviceResult))
                {
	...
	if ($insertDeviceResult)
        {
	 $status = 1;
      $errorMsg = "Successfully fetched the details of the user";
		 $data['upgrade'] =$upgrade;
	}
	else{
	
	$status = 0;
            $errorMsg = 'Error fetching details of the user';
		 $data['upgrade'] ="null";
	}
    ...
    public function GetUserDetails($UserData)
    {
        ...
        {
			$location2 = (!empty(htmlspecialchars(base64_decode($r['location'] )))) ? htmlspecialchars(base64_decode($r['location'] )) : htmlentities($r['location'] );
			$r['location']=$location2;
            $userDetails = $r;

            $temp=array();
			//$location2 = (!empty(htmlentities(base64_decode($location)))) ? htmlentities(base64_decode($location)) : htmlentities($location);
 ...
 if(  $errorMsg==""){
	 ...
	 public  function  AddUserImage($PostData)
    {
     ...
     $path="http://ns372321.ip-46-105-111.eu/~helpyaapp/HelpYa_WS/userimages/";
		 
        $bg = base64_decode($user_image);
		 
		 
		               
$imagepath=mysql_real_escape_string($path);
...
$imagestatus= "Error uploading image";
       ...
       "(user_id,user_image,image_path) ".
       "VALUES ".
       "('$user_id','$image_name','$imagepath')";
 ...
 if($errorMsg==""){
	...
	//echo $query;die();
        ...
        {
            $status = 1;
			$errorMsg = "Job details  inserted successfully";
        }
...
WHERE j.freelancer_id = ".$freelancer_id. " ORDER BY job_id DESC";
    ...
    public function GetAllReviews($UserData)
    {
        ...
        else if(strlen($max_review_id) > 0)
...
//        $message = array("content" => $_POST['message']);
    ...
    function MakePayment($PostData)
    {
    	...
    	$amount = addslashes(validateObject ($PostData , 'amount_id', ""));
    	$currency = addslashes(validateObject ($PostData , 'currency_id', ""));
    	$releaserequested=1;
		$paymentrelease=1;
    	$paidadmin=1;
     
	$query = "INSERT INTO `tblpayment`( `job_id`, `employer_id`, `freelancer_id`, `payfort_id`, `amount`, `currency`, `paid_to_admin`, `release_requested`, `payment_release`) VALUES (".$job_id.",".$employer_id.",".$freelancer_id.",".$payfort_id.",'".$amount."','".$currency."','".$paidadmin."','".$releaserequested."','".$paymentrelease."')";
		//$query = "INSERT INTO `tblpayment`(`job_id`, `employer_id`, `freelancer_id`, `payfort_id`, `amount`, `currency`, `paid_to_admin`, `release_requested`) VALUES(".$job_id.",".$employer_id.",".$freelancer_id.",".$payfort_id.",'".$amount."','".$currency."','".$paidadmin."','".$releaserequested."')";
 //print_r($query);die;
		$res = mysql_query($query) or $error =  mysql_error();
        if (!($res))
        {
        ...
        {
			$query2 = "UPDATE `tbljobs` SET `completed`= 1 WHERE `job_id` = ".$job_id;
			//print_r($query2);die;
					$res2 = mysql_query($query2) or $error =  mysql_error();
					if (!($res2))
        		{
            		$errorMsg = "Error in inserting payment details";
        		}
            		...
            		$status = 1;
			$errorMsg = "Payment details inserted successfully";
			$completed=1;
			}
    ...
    function UpdateRequestForJob($JobData)
    {
		...
		$query = "UPDATE tblpayment set release_requested = 1  where job_id = ".$job_id;
    			...
    			/*$query = "UPDATE `tblpayment` SET `release_requested`= 1,`payment_release`= 1 WHERE `job_id`= ".$job_id;
    ...
    function AddBankInfo($bankData)
    {
    	...
    	$code = addslashes(validateObject ($bankData , 'Bank/IFSCCode', ""));
    	$swift_code = addslashes(validateObject ($bankData , 'SwiftCode', ""));
    	$beneficiary_account = addslashes(validateObject ($bankData , 'IBANNo.*', ""));
    	...
    	$beneficiary_phone = addslashes(validateObject ($bankData , 'PhoneNumber', ""));
    	...
    	if(mysql_num_rows($response) == 0)
    	{
        	...
        	if (!($res))
        	{
           	 	$errorMsg = "Error in inserting bank details".$error;
       		}
        	else
        	{
          	  	$status = 1;
